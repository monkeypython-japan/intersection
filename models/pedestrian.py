...
GOAL_ALIGN_WEIGHT_DEFAULT = 2.0  # ゴール整合性スコアのデフォルト重み
...
class Pedestrian:
    def __init__(self, position: np.ndarray, goal_line: Boundary,
                 ...
                 goal_align_weight: float = GOAL_ALIGN_WEIGHT_DEFAULT):
        ...
        self.goal_align_weight = goal_align_weight
    ...
    def _plan_heading(self, forward: np.ndarray, nearby: list) -> float:
        ...
        scores = np.array([self.goal_align_weight * np.cos(c) for c in sector_offsets])
